...
class StreamTest extends FunSuite {
  ...
  test("filter") {
    assert(Stream(1, 2, 3, 4, 5).filter(x => x % 2 == 1).toList == Stream(1, 3, 5).toList)
  }

  test("flatMap") {
    val flatMappedStream = Stream(1, 2, 3, 4, 5)
        .flatMap(x => Stream(x - 1, x, x + 1))
        .toList
    assert(flatMappedStream == List(0, 1, 2, 1, 2, 3, 2, 3, 4, 3, 4, 5, 4, 5, 6))
  }
